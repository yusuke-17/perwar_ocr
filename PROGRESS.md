...
1. **G4 OCRの temperature/timeout 未指定** ← 次の候補
   OCR呼び出しに `options` がなく同じ画像でも結果がぶれる。全Ollama呼び出しに timeout が無く、
   固まると無限ハング（G2のフェイルファストは「失敗」は拾えるが「無応答」は拾えない）
2. **G3 変体仮名変換がパイプライン未接続** — テスト済み機能を繋ぐだけ（コスト小）
3. G5 検索インデックスが口語化後テキストのみ / G6 差分更新が meta.json の mtime のみ
4. G7 `process_single`/`process_batch` の重複解消（G2で `_run_ocr_page` / `_run_modernize` を
   共用化したので下地はできている）＋ E1 CLIテスト整備
5. 本丸: F1 チャット要約（ローカルRAG）→ F2 セマンティック検索 → F3 出典付き回答

### 保留メモ
- glm-ocr が画像によって ``` を1万文字ぶん繰り返す出力をすることがある（モデル側のループ）。
  OCR出力の異常検知（同一行の過剰反復を弾く等）は改善案に追加検討
...
- 設計の詳細は `plan/` 配下に別ファイルで置き、ここからはリンクするだけにする
