...
1. **G3 変体仮名変換がパイプライン未接続** ← 次の候補
   テスト済み機能を繋ぐだけ（コスト小）
2. G5 検索インデックスが口語化後テキストのみ / G6 差分更新が meta.json の mtime のみ
3. G7 `process_single`/`process_batch` の重複解消（G2で `_run_ocr_page` / `_run_modernize` を
   共用化したので下地はできている）＋ E1 CLIテスト整備
4. G9 口語化チャンクのフェイルファスト（G4で顕在化。無応答が続くと 300秒×チャンク数 待つ）
