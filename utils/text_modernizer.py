...
以下のルールに従い、文語体の表現をすべて口語体に変換してください。

【変換ルール】

■ 文語助動詞 → 口語
  なり → である    たり → であった    べし → べきである
  ず/ざり → ない/なかった    り → た/ている    けり → た
  ぬ → た    む（ん） → う/だろう    き → た
  まじ → まい    ごとし → ようだ
  候（そうろう） → です/ます/である

■ カタカナ文語 → ひらがな口語
  ハ→は  ヲ→を  ニ→に  ノ→の  ヘ→へ  ト→と
  スベシ→すべきである  セリ→した  ナリ→である
  ザルベカラズ→なければならない

■ 接続表現
  而して→そして  然れども→しかし  故に→そのため
  依りて→よって  即ち→すなわち  蓋し→思うに
  雖も（いえども）→とはいえ
...
■ その他
  哉（かな）→ ことだ/ものだ（感嘆を自然な口語に）
  之→これ  其→その  此→この

【制約】
- 固有名詞（人名・地名・流派名等）はそのまま保持する
- 原文の意味を正確に保つ。推測で補足しない
- 出力は変換後のテキストのみ。説明や注釈は不要"""

# Few-shot例（小さいモデルの指示追従を向上させるため）
# 出典: 大日本帝国憲法・教育勅語・陸軍省通達・歴史叙述文（いずれも公知の歴史的文書）
FEW_SHOT_EXAMPLES = [
    {
    ...
    },
    {
        ...
        "output": "その流祖は常陸国の人であり、はじめ心影流を学び、のちに自ら一流を開いた。"
        "その技量は非常に優れていたとはいえ、まだ満足とはしなかった。",
    },
]
